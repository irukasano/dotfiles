...
select_issue() {
  ...
  while IFS=$'\t' read -r number title labels_csv updated_at; do
    ...
    gh issue list --state open --assignee @me --limit 200 \
      ...
      --jq '.[] | [(.number | tostring), .title, ((.labels | map(.name)) | join(",")), .updatedAt] | @tsv'
...
select_pr() {
  ...
  while IFS=$'\t' read -r pr_number title branch_name author updated_at; do
    ...
    gh pr list --state open --limit 200 \
      ...
      --jq '.[] | [(.number | tostring), .title, .headRefName, .author.login, .updatedAt] | @tsv'
  ) | fzf --ansi --delimiter=$'\t' --with-nth=1
}
